...
(define (eval-definition line-obj)
  ...
  (if (ask line-obj 'empty?)
	...
	(cond ((not (symbol? name))
	       (logo-error "Procedure name is not a symbol" name))
	      ((lookup-procedure name)
	       (logo-error "Procedure definition already exist" name))
	      (else
...
(define (eval-macro-definition line-obj)
  ...
  (if (ask line-obj 'empty?)
      ...
      (let ((name (ask line-obj 'next)))
	(cond ((not (symbol? name))
	       (logo-error "Macro name is not a symbol" name))
	      ((lookup-macro name)
	       (logo-error "Macro definition already exist" name))
	      (else
	       (let ((formals (collect-formals)))
